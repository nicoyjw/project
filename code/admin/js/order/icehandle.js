Ext.define('Ext.ux.depotmanagerView',{
	...
	creatBbar: function() {
        ...
        var templatelist = this.arrdata[4];
		var thiz = this;
       return Ext.create('Ext.toolbar.Paging',{
			...
			items:['-',{
						  ...
						  text:'标签',
						   ...
						   handler:function(){
							   var ids = getIds(thiz.grid);
							    ...
							    Ext.Ajax.request({
									timeout:180000,
									url:'index.php?model=order&action=PrintIceLabel',
									loadMask:true,
									success:function(response){
										openWindowWithPost(response.responseText,ids,'订单打印'+Math.floor(Math.random()*100));
									},
									failure:function(response){
					   ...
					   },'-','<a onclick="window.open(\'http://kd.szice.net:8086/xms/\')" href="#">互联易登录</a>','-','<a onclick="window.open(\'http://61.144.222.46/blog/?p=1991\')" href="#">查看渠道</a>']				   
        });
	...
	update4px:function(type){
		...
		Ext.Ajax.request({
			timeout:180000,
			url:'index.php?model=order&action=getIceTrackNo',
			loadMask:true,
					success:function(response){           
						 Ext.Msg.show({
                           title: '提示 ',
